...
containing this `SKILL.md`, and resolve `<skills-dir>` to its parent directory.
Keep `--project-root` pointed at the active novel workspace, not at the skill or
plugin installation.
...
```bash
python3 "<skills-dir>/novel-story-telling/scripts/check_continuity.py" \
  --project-root . \
  --strict
```
...
```bash
python3 "<skill-dir>/scripts/build_epub.py" \
  --project-root .
```
