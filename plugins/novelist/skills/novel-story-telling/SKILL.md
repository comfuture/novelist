...
containing this `SKILL.md`. Keep `--project-root` pointed at the active novel
workspace, not at the skill or plugin installation.
...
```bash
python3 "<skill-dir>/scripts/build_story_context.py" \
  --project-root . \
  --chapter <number> \
...
```bash
python3 "<skill-dir>/scripts/check_continuity.py" --project-root .
```
...
```bash
python3 "<skill-dir>/scripts/write_chapter.py" \
  --project-root . \
...
```bash
python3 "<skill-dir>/scripts/check_continuity.py" \
  --project-root . \
  --strict
```
...
```bash
python3 "<skill-dir>/scripts/update_story_ledger.py" \
