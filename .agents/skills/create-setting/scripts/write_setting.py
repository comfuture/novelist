#!/usr/bin/env python3
"""Write an el_empiezo world/setting markdown file from JSON."""
...
def normalize(data: dict[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    frontmatter = {
        "id": data.get("id") or f"world-{slug}",
        "type": "world",
        "title": title,
        "category": data.get("category", "setting"),
...
def render_body(data: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "## Description",
            "",
            body_text(body.get("description"), "Describe the place, time, institution, rule, or world-state."),
            "",
            "## Story Pressure",
            "",
            body_text(body.get("story_pressure"), "How does this world detail create constraint, opportunity, or conflict?"),
            "",
            "## Continuity",
            "",
            body_text(body.get("continuity"), "Track facts that should not drift between chapters."),
            "",
        ]
    )
...
def main() -> int:
    ...
    parser.add_argument("--input", required=True, type=Path, help="Setting JSON file")
    ...
    output = args.project_root / "world" / f"{slug}.md"
